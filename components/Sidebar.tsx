    ...
    <>
      ...
      <aside className="hidden md:flex flex-col fixed left-0 top-0 h-screen bg-transparent border-r border-white/[0.04] z-40 transition-all duration-300 w-20 lg:w-64 py-8 select-none">
        ...
        <div className="flex items-center gap-3 px-6 mb-10 text-white">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-tr from-accent-indigo via-accent-violet to-accent-cyan text-white shadow-[0_0_20px_rgba(99,102,241,0.2)]">
            ...
            return (
              <button
                ...
                className={`w-full relative flex items-center gap-4 py-3 px-4 rounded-xl text-sm font-medium transition-colors duration-200 group outline-none cursor-pointer ${
                  isActive ? "text-accent-indigo" : "text-white/50 hover:text-white/80"
                }`}
              >
                ...
                {isActive && (
                  <motion.div
                    ...
                    className="absolute inset-0 bg-gradient-to-r from-accent-indigo/80 to-transparent rounded-xl border-l-[3px] border-accent-indigo"
                    style={{
                ...
                <div className="relative flex items-center justify-center">
                  <Icon className={`h-5 w-5 transition-transform duration-200 ${
                    isActive ? "scale-105 text-accent-indigo" : "group-hover:scale-105"
                  }`} />
                  
                  {/* Subtle Glowing active point */}
                  {isActive && (
                    <span className="absolute -top-1 -right-1 flex h-1.5 w-1.5">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-accent-indigo/60 opacity-75"></span>
        ...
        <div className="px-4 border-t border-white/[0.04] pt-6 mt-auto">
          <div className="flex items-center gap-3 py-2 px-3 rounded-xl hover:bg-white/[0.02] border border-transparent hover:border-white/[0.03] transition-all duration-300 group cursor-pointer">
            <div className="h-9 w-9 rounded-full bg-gradient-to-br from-accent-violet/20 to-accent-indigo/20 border border-white/10 flex items-center justify-center text-xs font-semibold text-white/90">
              VS
            </div>
          ...
          return (
            <button
              ...
              className={`relative flex flex-col items-center justify-center flex-1 h-full py-1 text-xs font-medium outline-none cursor-pointer ${
                isActive ? "text-accent-indigo" : "text-white/40 hover:text-white/70"
              }`}
            >
              ...
              {isActive && (
                <motion.div
                  ...
                  className="absolute bottom-1.5 h-1 w-8 rounded-full bg-accent-indigo"
                  transition={{ type: "spring", stiffness: 350, damping: 25 }}
                />
              )}

              <Icon className={`h-5 w-5 mb-1 transition-transform duration-200 ${
                isActive ? "scale-105 text-accent-indigo" : ""
              }`} />
